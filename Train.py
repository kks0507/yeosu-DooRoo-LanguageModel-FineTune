...
MODEL_REPO = "kingkim/Dooroo2025_v1.0"   # 학습 결과(모델)를 업로드할 저장소
...
# =================================================================================
# =================================================================================
# 7. 모델 병합, 로컬 저장, 그리고 Hub 업로드 (수정된 최종안)
# =================================================================================
from huggingface_hub import HfApi

# Hugging Face Hub에 생성될 레포지토리 정보
hf_repo_id = MODEL_REPO # "kingkim/Dooroo2025"

# --- 1. LoRA 어댑터를 베이스 모델과 병합합니다 ---
print("\nLoRA 어댑터를 베이스 모델과 병합합니다...")
model = model.merge_and_unload()
print("병합 완료.")

# --- 2. [핵심] 병합된 모델과 토크나이저를 로컬 폴더에 함께 저장합니다 ---
merged_model_path = "./Dooroo2025_complete01" # 저장할 로컬 폴더 이름
print(f"병합된 모델과 토크나이저를 로컬 경로 '{merged_model_path}'에 저장합니다...")

model.save_pretrained(merged_model_path)
tokenizer.save_pretrained(merged_model_path) # ⬅️ 토크나이저도 같은 폴더에 저장하는 것이 핵심!

print("로컬 저장 완료.")

# --- 3. [핵심] HfApi를 사용하여 로컬 폴더 전체를 Hub에 업로드합니다 ---
print(f"'{merged_model_path}' 폴더의 모든 내용을 '{hf_repo_id}' 레포지토리에 업로드합니다...")
print("이 작업은 시간이 다소 걸릴 수 있습니다.")

api = HfApi()
api.upload_folder(
    folder_path=merged_model_path, # 업로드할 로컬 폴더 경로
    repo_id=hf_repo_id,            # 대상 허깅페이스 저장소
    repo_type="model",
    token=HF_TOKEN,
)
