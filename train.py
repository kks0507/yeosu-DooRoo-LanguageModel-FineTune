...
# =================================================================================
# 4. 데이터셋 준비 (Unsloth 스타일 적용) — 안전한 파서로 수정본
# =================================================================================
import json
from unsloth.chat_templates import get_chat_template
from datasets import load_dataset
...
# 3) 'messages'를 안전하게 파싱하여 chat template 적용
...
# 5) 빈 문자열 제거 (파싱 실패 샘플 정리)
dataset = dataset.filter(lambda x: x["text"] != "")

# 6) 스플릿 준비 (validation 이름 주의)
...
model.push_to_hub(
    "kingkim/yeosu_dooroo_2", # 레포지터리 ID
    tokenizer = tokenizer,
    token = HF_TOKEN,
)
